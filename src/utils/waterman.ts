...
const findAlignmentPaths = (maxScoreCells: Cell[]) => {
  const paths: Cell[][] = [];
...
const findAlignments = (paths: Cell[][], string1: string, string2: string, matchScore: number, mismatchScore: number, penalty: number) => {
  const alignments = [];
...
const findTableCellsAndMaxScoreCells = (string1: string, string2: string, matchScore: number, mismatchScore: number, penalty: number) => {
  const cells = [];
  const maxScoreCells = [];
  let maxScore = 0;
  for (let i = 0; i < string1.length + 1; i++) {
      ...
      for (let j = 0; j < string2.length + 1; j++) {
          ...
          const newCell: Cell = { score: newScore, parents: parents, rowIndex: i, colIndex: j };
...
const allPathsFinished = (paths: Cell[][]) => {
...
const getWatermanMatrixCells = (string1: string, string2: string, matchScore: number, mismatchScore: number, penalty: number) => {
